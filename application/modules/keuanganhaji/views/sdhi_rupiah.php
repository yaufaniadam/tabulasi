<section class="content-header">
	<h1><i class="fa fa-kaaba"></i> Laporan SDHI Rupiah <a
			href="<?= base_url('keuanganhaji/tambah_sdhi_rupiah'); ?>" class="btn btn-warning btn-sm"><i
				...
				<tr>
					...
					<td class="text-center">
						...
						<a style="color:#fff;" title="Lihat Detail" class="btn btn-xs btn-info"
							href="<?= base_url($this->router->fetch_class() . '/detail_sdhi_rupiah/' . $row['bulan'] .'/' .$row['tahun']); ?>">
							<i class="fa fa-eye"></i></a>

						<a style="color:#fff;" title="Hapus" class="delete btn btn-xs btn-danger"
							data-href="<?= base_url($this->router->fetch_class() . '/hapus_sdhi_rupiah/' . $row['bulan'].'/' .$row['tahun']); ?>"
							data-toggle="modal" data-target="#confirm-delete"> <i class="fa fa-trash-alt"></i></a>
							<a href="<?= base_url('keuanganhaji/export_sdhi_rupiah/'. $row['bulan'] .'/' . $thn); ?>" class="btn btn-success btn-xs"><i
