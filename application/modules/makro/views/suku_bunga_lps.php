<section class="content-header">
  <h1><i class="fa fa-file-pdf"></i> &nbsp; Suku Bunga LPS Indonesia
        ...
        <tbody>
          <?php foreach ($suku_bunga_lps as $row) { ?>
            <tr>
              ...
              <td class="text-center"><a style="color:#fff;" title="Hapus" class="delete btn btn-xs btn-danger" data-href="<?=base_url('makro/hapus_suku_bunga_lps/'.$row['id']); ?>" data-toggle="modal" data-target="#confirm-delete"> <i class="fa fa-trash-alt"></i></a></td>
...
<script type="text/javascript">
  ...
  if ($("#form_suku_bunga_lps").length > 0) {
    $("#form_suku_bunga_lps").validate( {
      rules: {
        tahun: {
          required: true,
          minlength: 2
      ...
      submitHandler: function(form) {
        $.ajax({
          url: SITEURL + "makro/tambah_suku_bunga_lps",
          data: $('#form_suku_bunga_lps').serialize(),
          type:"post",
          dataType: 'json',
          success: function(res){
             var suku_bunga_lps = '<tr id="id_' + res.data.id + '"><td>' + res.data.tahun + '</td><td>' + res.data.suku_bunga_lps + '</td><td class="text-center"><a style="color:#fff;" title="Hapus" class="delete btn btn-xs btn-danger" data-href="' + SITEURL + '/makro/hapus_suku_bunga_lps/' + res.data.id + '" data-toggle="modal" data-target="#confirm-delete"> <i class="fa fa-trash-alt"></i></a></td>';


              $('#table').prepend(suku_bunga_lps);          
